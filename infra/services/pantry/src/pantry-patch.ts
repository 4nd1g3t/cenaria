...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  // notes (string => set, null => remove, undefined => no-op)
  ...
  try {
    const res = await ddb.send(new UpdateCommand({
      ...
      ...(expectedVersion !== undefined
        ? { ConditionExpression: '#v = :expected', ExpressionAttributeValues: { ...vals, ':expected': expectedVersion } }
